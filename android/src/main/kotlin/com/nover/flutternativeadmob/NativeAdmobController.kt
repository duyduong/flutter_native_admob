...
class NativeAdmobController(
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (CallMethod.valueOf(call.method)) {
            CallMethod.initAd -> {
                val id = call.argument<String>("adUnitID") ?: return result.success(null)
                val keywords = call.argument<List<String>>("keywords")

                val isChanged = adUnitID != id
                adUnitID = id
                
                val adRequestBuilder = AdRequest.Builder();
                keywords?.forEach {
                    adRequestBuilder.addKeyword(it);
                }
            ...
            CallMethod.reloadAd -> {
                call.argument<Boolean>("forceRefresh")?.let {
                    if (it || nativeAd == null) loadAd(adRequest) else invokeLoadCompleted()
                }
            }
        }
    }
